...
public class ExcerciesAdapter_Log extends BaseAdapter {
    ArrayList<ExcerciseModel> elist = new ArrayList<>();
    Context ctx;
    Dialog dialog;
    public ExcerciesAdapter_Log(ArrayList<ExcerciseModel> elist, Context ctx) {
        this.elist = elist;
        this.ctx = ctx;
    }
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
       ...
       // desc.setText(em.getDesc());
        Picasso.get().load(em.getImage()).error(R.drawable.excer).into(icon);
               // with(ctx).load(em.getImage()).error(R.drawable.excer).into(icon);
